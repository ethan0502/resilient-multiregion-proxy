...
A separate real-world data point, unrelated to the repeated-run test above: three Ookla Speedtest runs from a phone on 5G through the Tokyo node's exit, spanning the hour the blue-green front door went live there. The middle result (53.6 Mbps upload) was captured immediately after a full server reboot and was transient; upload fell back below 1 MB/s soon afterward. The post-cutover result shows the refreshed-process performance without requiring a disruptive whole-server reboot. See the root [README's version of this section](../README.md#upload-throughput-around-the-blue-green-cutover) for the full table and chart, and [`docs/blue-green-deployment.md`](../docs/blue-green-deployment.md#motivation) for the degradation mechanism. Timestamps were cross-checked against the node's own deployment artifacts (config file mtimes, flip log) rather than taken at face value.
