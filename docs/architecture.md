...
This document describes each node's runtime shape, why the runtimes are not interchangeable, and the production incident that motivated hardening the front-door process supervision. All IPs are placeholders (`203.0.113.0/24`, an RFC 5737 documentation range); all keys, client identifiers, and handshake short-IDs are placeholders.

## Node inventory

| Node | Public `443` front door | Backend supervision | Notable constraint |
|---|---|---|---|
| **Node A** (Tokyo) | nginx `stream` module, TCP passthrough, blue-green | paired backend containers | — |
| **Node B** (Malaysia) | HAProxy, TCP passthrough, blue-green | paired systemd backend units | Plesk-managed box; bundled nginx build has no compatible stream module in its enabled repos, so HAProxy owns 443 instead |
...
Every node is generated from the same config shape: one authenticated TCP inbound with a camouflaged TLS handshake and no top-level routing policy by default. Two backend processes per node are produced from a single source-of-truth config file by a small generator script that copies everything **except** the listen port — so the two backends cannot drift apart on cryptographic identity, client lists, or handshake parameters.

### Node A also carries a legacy runtime

Node A additionally runs an older systemd-managed server instance on a non-standard port, kept alive during the migration to the containerized blue-green runtime so existing clients weren't force-migrated. The two runtimes are **not the same source of truth** — different config file, client list, and cryptographic identity — and the CLI tooling in this repo only targets the legacy systemd-managed one. This distinction (which config is actually live, and which tool actually manages which config) had to be documented explicitly and re-verified repeatedly, because assuming they matched was the direct cause of at least one client-visible outage.
...
1. The kernel OOM killer selected the running proxy backend process under memory pressure and killed it.
...
3. The backend's own restart attempts then failed with `bind: address already in use`; after several rapid failures, systemd gave up with "start request repeated too quickly."
...
One node's own upstream peering to a specific destination network is poor — confirmed by isolating the proxy stack entirely and measuring raw, direct transfer throughput over the identical path, which reproduced the same slow number, proving the bottleneck was the network path itself and not the proxy stack. A different node happens to have excellent peering *to* that same destination. Rather than accept the slow direct path, a two-hop relay was built: a client connects to the well-peered node's dedicated relay-only inbound (with a separate cryptographic identity from normal client traffic), which re-encapsulates the connection as a raw TCP client of the target node's egress-only fast path, exiting as the target node's IP.
